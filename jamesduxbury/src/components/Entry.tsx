...
export const Entry: React.FC<{ settings: SiteSettings }> = ({ settings }) => {
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-[auto_1fr] sm:items-start sm:gap-12">
          ...
          <div className="divide-y divide-border border-y border-border">
            ...
            <SpecRow label="Role">{settings.entryRole}</SpecRow>

            <SpecRow label="Education">{settings.entryEducation}</SpecRow>

            <SpecRow
              label="Years"
              trailing={
                <span className="font-mono text-xs text-muted">{settings.entryStatus}</span>
              }
            >
              <span className="font-mono">{settings.entryYears}</span>
            </SpecRow>
            ...
            <SpecRow
              ...
              trailing={
                <Link
                  href={cvHref(settings)}
                  download={cvDownloadName(settings)}
